...
then
	...
	if cat diff | grep 'Listen to' >/dev/null		# checks to see if in the difference file there is the text Listen to, meaning there is new music on blonded.co (the >/dev/null suppresses the terminal output)
